...
%
In the fuzzer's perspective, while instrumentation at memory access
instructions has negative impacts on throughput, it provides a higher
chance for a fuzzer to execute more interesting inputs (\ie,
interesting thread interleaving) and not to waste computing resources.
...
\end{table}
%
To closely examine \sys's overheads, we measure the elapsed time
required to execute one input that .
