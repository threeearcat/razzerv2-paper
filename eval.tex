...
\begin{table*}
  ...
  \caption{Result of the comparison study against various
    ...
    number of executions and the elapsed time (in ms) required to
    discover each concurrency bug. The \texttt{Naive} column indicates
    the kernel scheduler (\ie, no thread scheduling control applied).}
  \label{table:comparison-interleaving-search}
\end{table*}
%
Here, we measure the elapsed time and number of executions required to
discover a given concurrency bug for each approach.
...
single-thread fuzzing~(\autoref{ss:singlethreadfuzzing}), and manually
provide a multi-thread input as well as a pair of system calls that
can cause each concurrency bug. Then, we observe how each fuzzer
explore thread interleavings of a given pair of system calls.
%
In this way, we can focus on the effectiveness of the thread
interleaving exploration of each fuzzer without being interrupted by
the randomness. \dr{N times of executions.}
...
result.
%
As the table shows, the difficulty of discovering varies from bug to
bug. For example, CVE-2018-12232 appears the easiest concurrency bug
to discover since it can be discovered within 40 executions even
without any thread scheduling control mechanism.
%
On the other hand, three concurrency bugs, CVE-2019-6974,
CVE-2019-11486, and \texttt{69e16d01d1de}, hard to discover with the
kernel scheduler.



Regardless of the difficulty of discovering concurrency bugs, \sys can
discover all of concurrency bugs extremely fast.
\dr{wip.}

Compared to the kernel scheduler (\ie, the \texttt{Naive} column)
...
%
We also justify the choice of our interleaving coverage by
demonstrating what happens if a fuzzer adopts less-informative
...
\begin{table}[t]
  ...
  \caption{
    ...
    (\texttt{Exec. syscall}), computational overheads
...
benefit in exploring thread interleavings is huge, we still believe
that this is a tolerable result.
\dr{}
