...
run \sys and prior approaches with fixed system calls that can cause
...
fuzzer.
%
Then, we measure the number of executions and the elapsed time until
each fuzzer triggers each concurrency bugs within a given pair of
system calls. During the evaluation, we limit the number of executions
to 10000.
...
\subsubsection{Trade-off between the bug-finding capability and the
  search complexity}
\label{sss:interleavingcoverage}
%
In order to answer the question why a concurrency fuzzer adopts an
informative interleaving coverage metric, we compare interleaving
segment coverage and less-informative interleaving coverage (\eg,
alias coverage~\cite{krace}).
%
We expect that interleaving segment coverage is not saturated until a
...
mimick alias coverage in \sys by limiting the size of each
interleaving segment to two. In other words, interleaving segments of
size 2 describe interleavings of two instructions in which at least
one is a write instruction.
%
Using mimicked alias coverage and interleaving segment coverage as a
...
When using mimicked alias coverage, \textit{many (six ouf of nine)
  concurrency bugs are not triggered in ten trials even if the
  mimicked alias coverage is saturated}.
%
In \autoref{s:motivation}, with an example of CVE-2017-17712, we
provide how alias coverage is saturated even before the concurrency
bug is triggered.
%
Whereas, all listed concurrency bugs are triggered before interleaving
segment coverage is saturated.
%
...
On the other hand, alias coverage is saturated really quickly. It is
saturated in 13.9 executions on average, ranging from 6 to 32.
%
Whereas, interleaving segment coverage requires more executions, (at
least) 27 executions on average, ranging from 3 to 81.
...
informative to describe unique behaviors of thread interleavings, its
search space is too large to explore, and thus, it is impractical.
