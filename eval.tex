...
We compare \sys against prior approaches in the perspective of the
following: 1) is interleaving segment coverage~(\autoref{ss:coverage})
really informative and helpful in discovering concurrency bugs?
(\textbf{Design goal 1} in
...
For this evaluation, we select well-known concurrency bugs, and run
fixed system calls that can cause each concurrency bug using \sys and
previous approaches.
...
%
We justify our design choice of interleaving segment coverage by
showing the trade-off between the bug-finding capability and the
search complexity.
...
\subsubsection{Comparison of interleaving exploration methods}
