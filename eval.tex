...
\begin{table*}
  ...
  \caption{Result of the comparison study against various
    ...
    Krace~\cite{krace} and Snowboard~\cite{snowboard}). The
    \texttt{Naive} column indicates the kernel scheduler (\ie, no
    thread scheduling control applied). \dr{}}
...
and Snowboard \dr{does not utilize KVM}.
%
...
random delay injection scheme of Krace, and 2) the \dr{}PMC-test scheme of
Snowboard.
%
...
seeds~\cite{fuzzingeval}.
%
\dr{wip.}

\dr{say: one but at a time, seed fix (disble single-thread fuzzing), N
  times of executions,}
...
%
We also justify the choice of our interleaving coverage by comparing .
