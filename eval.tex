...
We conduct a comparison study \sys against various prior approaches to
demonstrate the performance improvement \sys in discovering
concurrency bugs.
%
In particular, 1) we compare the \sys's interleaving search strategy
with methods of previous interleaving search
strategies\dr{}~(\autoref{sss:interleavingsearch}), and 2) an
...
bugs (\ie, data races), we exclude concurrency bugs used in Krace,
because we do not have access to those concurrency bugs. Also, we
exclude two Android-related concurrency bugs (\ie,
...
execute Android-specific code spaces where \sys (and
\texttt{Syzkaller}) are limited to exploring.
...
\begin{table}[t]
  ...
  \caption{
    %
    Elapsed time (ms) for executing one multi-thread input, and
    overheads of each components of \sys.}
