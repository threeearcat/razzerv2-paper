...
interleaving. Suppose two unexplored segment graphs are given as
described in \autoref{fig:hint}-(a).


, and \autoref{fig:hint}-(b)
...
subset of $G_{mutated}$, $G^*$.
%
Specifically, we start with an empty set $G^*$. Then, we iterate
over mutations in $g^* \`in G_{mutated}$. With $g^*$, we determine 

and determine adding each segment graph into $G^*$ forms a loop or
not. If it does not form a loop, we add the segment graph into $G^*$.
%
After iterating all mutated segment graphs in $G_{mutated}$, $G^*$
does not contain a loop. The fuzzer then excludes selected segment
graphs from the set of all mutated segment graphs (\ie,
$G_{mutated} = S_{mutated} \setminus G^*$), and generates scheduling
points to explore $G^*$.


One important constraint of $G^*$ is 
