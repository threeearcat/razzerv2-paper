...
elements of $U^*$.
%%
If $U^*$ is not empty, we mutate the interleaving toward scheduling
constraint pairs in $U^*$.


Lastly, we mutate the interleaving \textit{directed towards scheduling
  constraint pairs in $U^*$}.
%

Since scheduling constraint pairs in $U^*$

In \autoref{fig:overview}-(c), \texttt{Segment 1'} and
\texttt{Segment 2'} are exclusive with each other; a single
interleaivng cannot fulfill these two scheduling constraint pairs as
they make a contradiction on the executin order of \texttt{A1} and
\texttt{B1}.
%
However, \texttt{Segment 1'} \texttt{Segment 3'} are not exclusive to
each other; we can imagine an instrucion sequence
