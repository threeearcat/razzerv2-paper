...
%
\autoref{fig:interleavingsegmentgraph} illustrates how to compute
segment graphs with the execution example of
\autoref{fig:keyidea}-(a).
%
The first step is to represent the execution as a DAG. Since the
...
dotted edges (\eg,
$\texttt{A2} \Rightarrow \texttt{A4}, \texttt{A4} \Rightarrow
\texttt{A6}$) are drawn to represent orderings in a \textit{single}
thread, such as ``\textit{\texttt{A2} is executed before \texttt{A4}
  in thread~A}''.
%
...
example, $\texttt{B1} \Rightarrow \texttt{A2}$ represents interleaving
between \texttt{B1} and \texttt{A2} that access the same memory object
(\ie, \texttt{inet->hdrincl}).
%
...
$G = \{g_1, g_2, ..., g_n\}$, where $g_i \in G$ is a segment graph
that has two interleaving-order edges.
%
