...
concurrent job are given as an input. We also assume that all memory
access operations are totally ordered.
...
%
Based on the observation, we can schedule instructions in a
\textit{smart} way.
%
In other words, if we know which fragments are not yet observed from a
universal fragment set $U$, 
