...
iterations.
%
In this subsection, we explain how we generate scheduling points, and
quickly saturate interleaivng segment coverage.
...
\autoref{fig:hint}) are used to infer unexplored interleaving segments
...
%
For each segment graph $g_i$, the fuzzer permutes interleaving-order
edges in $g_i$, and generates $S^{i}_{derived}$, a set of derived
segment graphs.
%
Specifically, the fuzzer enumerates interleaving segment graphs
generated by permuting interleaving-order edges.
...
\begin{figure}[t]
  ...
  \caption{Example interleaving segment graphs derived from
    \texttt{Segment \#1}. The fuzzer will not utilize (b) and (c)
    becuase (b) is already explored, and (c) contains a loop.}
  \label{fig:interleavingmutation}
\end{figure}
%


\autoref{fig:interleavingmutation} illustrates examples of derived
segment graphs.
%
Given \texttt{Segment \#1}, the fuzzer permutes interleaving-order
edges (denoted as solid edges), and generates three segment graph
examples.
%
In particular, from \texttt{Segment \#1}, the fuzzer flips
$\texttt{A4} \Rightarrow \texttt{B1}$ in (a), does not touch
interleaving-order edges in (b), and flips
$\texttt{A2} \Rightarrow \texttt{B1}$ in (c).
...
%
The fuzzer also excludes ones that contain a loop because it is
impossible to enforce the execution order because a loop indicates a
contradiction on the execution order; a loop indicates that any vertex
...
and unexplored segment graphs) can be selecting a number of
interleaving segments contained in $S_{derived}$, and find an
interleaving instance to cover selected interleaving segments.

To this end, the fuzzer selects its subset and composes an imaginary
interleaving instance using interleaving segments
%
