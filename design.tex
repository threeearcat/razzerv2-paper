...
a fuzzer about which interleaving segments need to be tested in future
iterations.
%
\autoref{fig:hint} demonstrates how an observed interleaving segment
can be helpful for future iterations.
%
With \texttt{Segment \#1} from \autoref{fig:keyidea-b}, a fuzzer can
derive \textit{unobserved interleaving segments} by rearranging
instructions in the interleaving segment.
%
As illustrated in \autoref{fig:hint}, a fuzzer can derive unobserved
\texttt{Segment \#1-a} by imagining an execution scenario that
\texttt{B1} is executed before \texttt{A2}.
%
Similarly, \texttt{Segment \#1-b} can be derived by changing the
execution order of \texttt{A4} and \texttt{B1}.
%
...
iterations. If a fuzzer focuses on searching these two interleaving
segments, it will quickly discover the uninitialized bug of
\autoref{fig:cve-2019-6974} when it executes \texttt{Segment \#1-b}.
%
...
\begin{figure}[t]
  ...
  \caption{\texttt{Segment \#1} is an observed segment in
    \autoref{fig:keyidea}, and \texttt{Segment \#1--a} and
    \texttt{Segment \#1}--b are \textit{unobserved} interleaving
    segments derived from \texttt{Segment \#1}.}
  \label{fig:hint}
\end{figure}
%
