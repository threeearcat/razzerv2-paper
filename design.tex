...
In order to explore thread interleavings of a multi-thread input, our
approach takes an \textit{explored} thread interleaving as a seed
interleaving.
%
...
\end{enumerate}
%
In our approach, decomposed interleaving segments will be tracked by
an interleaving coverage metric. based on mutating interleaving
segments and recomposing muteated segments.
...
%
Then, we illustrate how we design an interleaving coverage metric to
express combinatorial interleavings (\textbf{Design goal
  1})~(\autoref{ss:coverage}).
%
Lastly, we explain our speculative interleaving exploration method 

how
mutating interleaving segments and recomposing a new thread
interleaving can help discover thread interleavings effectively
