\section{Decomposing Thread Interleaving Space}
\label{s:design}
...
In the following, we first explain our key idea behind why we
decompose thread interleaving into \textit{interleaving
...
\dr{TODO:
  1. why do we decompose thread interleavings?
  2. how do we do that (e.g., four instructions)?
}
...
%
\dr{explain the decision choices (single interleaving, ...)}
%
...
(represented in \texttt{Segment \#1}), a fuzzer easily realizes that
there are more possible execution orders of the three instructions via
...
In this subsection, we explain how we decompose an explored thread
interleaving into interleaving segments.
%
We represent a thread interleaving as a directed acyclic graph
...
%
\Intcov is a collection of segment graphs, designed to track
interleaving orders within an interleaving segment, and to guide a
fuzzer to efficiently search possible instruction interleavings.
...
represents an execution ordering between two memory-access
...
derived from a interleaving.
%
Because the given interleaving described in
...
interleaving, and computing subgraphs of $g$ (\ie, segment graphs).
...
\dr{redundant}
%
Semantically, segment graphs represent a combination of interleaving
...
\subsection{Coverage-directed Interleaving Exploration}
\label{ss:scheduler}
...
Our interleaving exploration is \textit{not random} unlike previous
approaches~\cite{krace, ski, pctalgorithm, muzz}, Rather, it is a
systematic exploration \textit{directed by explored interleaving
  coverage}.
%
...
% iterations.
% %
In this subsection, we explain how we utilize explored segment graphs
to systematically and effectively explore thread interleavings.
...
\PP{Recomposing a thread interleaving}
%
