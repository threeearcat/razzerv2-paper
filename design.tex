...
coverage metric to achieve \textbf{Design goal
  1}~(\autoref{ss:coverage}) and the instruction scheduling mechanism
to quickly saturate the coverage metric (\eg, satisfying
...
a concurrency bug.
%
Taking the example of \autoref{fig:cve-2017-17712}, the uninitizlied
...
%
If one searches for all thread interleavings, he or she is able to
discover all concurrency bugs with a cost of astronomical time, and if
one focuses on the execution order of at most two instructions, he or
she quickly gets his job done with a risk of a high chance of missing
many (\eg, about 70\%) concurrency bugs.
%
...
\begin{figure}[t]
  \subfloat[Thread interleaving\label{fig:keyidea-a}]{
  ...
  \caption{(a) A thread interleaving example of
    ...
    contained in the thread interleaving. Note that we intentionally
