...
%
\dr{TODO: fix figure 2 to represent failure-oblivious instructions and
  use the instructions as an example of this case}
...
\dr{change this step into two?: feedback from new coverage ->
  pridicting undisclosed coverage}
%
...
only collect memory access operations conducted by the concurrent job,
...
scheduling constraint $SC$ as a directed pair ($M_i, M_j$) if
$M_i(tid) \neq M_j(tid)$, $M_i(addr) = M_j(addr)$,
$M_i(op) = store\; or\; M_j(op) = store$, and
...
\PP{Feedback mechanism}
%
\dr{TODO: rename this paragraph}
%
An interleaving finding new biconflict coverage likely exposes a
behavior that was unrevealed before.
%
Therefore, if new biconflict coverage is found, the fuzzer mutates the
interleaving to discover more interesting behaviors. \ie, the fuzzer
reschedules instructions to conform with other interleavings.

Before mutating an interleaving, the fuzzer predicts \textit{XXX}, a
set of scheduling constraint pairs that has not been observed but is
expected to occur when mutating the interleaving.
%
A \textit{XXX} will be used to direct the interleaving mutation. \ie,
the fuzzer will try to generate interleaivngs to expose undisclosed
scheduling constraint pairs instead of blindly mutating an
interleaving.
%

\textit{XXX} is derived from biconflict coverage $BC\mbox{-}cov$,
$\{SCPair_1, SCPair_2, ..., SCPair_n\}$.
%
For each $SCPair_i$ composed of $(SC_x, SC_y)$, the fuzzer derives
