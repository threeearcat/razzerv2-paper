...
%
\dr{i < j is to eliminate duplicates. (SC_i, SC_j) and (SC_j, SC_i)}
%
...
$\{SCPair_1, SCPair_2, ..., SCPair_n\}$, defined from its memory
...
%
An interleaving finding new biconflict coverage may expose a behavior
that was unrevealed before.
%
Therefore, if new biconflict coverage is found, we mutate the
interleaving to discover more interesting behaviors.


To this end, we adopt a mutation strategy \textit{directed} by
\textit{XXX}.
%
A \textit{XXX} is a scheduling constraint pair that has not been
observed, but is expected to occur when mutating the interleaving.
%
