...
\newcommand{\coverage}{biconflict coverage\xspace}
...
subject of mutation if the input exhibits new coverage. In our
context, an interleaving could be a subject of mutation if new
concurrency coverage (\eg, \coverage) is observed.
%
...
%
Our intuition comes from an observation that the execution order of
conflicting accesses may affect an outcome of a program.
%
Based on the observation, for each coverage, we change the execution
order of conflicting accesses to construct new \coverage that is
possibly observed by another interleaving, called \coverage candidate.
