...
%
We represent an explored thread interleaving as a directed acyclic
graph (DAG). Then, we compute subgraphs of the DAG called segment
graphs, each of which represents an interleaving segment.



Using a set of segment graphs, we propose a novel interleaving
coverage metric called \textit{\intcov}. \Intcov is designed to track
interleaving orders within an interleaving segment and used for
guiding a fuzzer to efficiently search possible instruction
interleavings.  If per-input \intcov is saturated, a fuzzer can
confidently conclude that the current input unlikely causes a
concurrency bug and proceed to the next input.
