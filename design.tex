...
%
Lastly, we explain how to mutate (\ie, \textit{step 2}) and recompsoe
...
%
Each interleaving segment represents an interleaving of a small number
of instructions accessing shared memory objects.
%
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.55\linewidth]{fig/interleavingmutation.pdf}
...
\begin{figure}[t]
  ...
  \caption{Example of recomposing mutated segment graphs into a whole
    thread interleaving, where (a) represents mutated segment graphs,
    (b) represents a graph combining the two mutated segment graphs,
    and (c) represents an instruction sequence to test \texttt{Segment
      \#1*} and \texttt{\#2*}}
  \label{fig:hint}
  \vspace{-5pt}
\end{figure}
