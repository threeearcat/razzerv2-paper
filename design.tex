...
For a thread interleaving, $G = \{g_1, g_2, ..., g_n\}$
...
To hash an interleaving graph, we define a hash function $hash(g)$ for
an interleaving graph $g$.
%
The key property of the $hash(g)$ is reflecting directions of edges,
...
To this end, we adopt an idea of Merkel hashing~\cite{treehashing,
  treehashing2}.
%
In particular, for all vertices $v$ in $g$, we first calculate a hash
value of $v$ based on its out-going edges such that
$hash(v) = \mathcal{H}(v ++ )$, where $\mathcal{H}$ denotes the
non-cryptographic FNV hash function~\cite{fnv, fnv-go}, and $++$
denotes the concatenation operation.
%
Then, $hash(g) = \underset{v \in g}{\oplus} hash(v)$, where $\oplus$
