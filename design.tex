...
second step is to further combine instructions pairs into an
interleaving segment.
%
Since each pair contains two memory accesses, we select two pairs when
combining them in order to make interleaving segments contain at most
four instructions.
%
For example, we select two instruction pairs
$\texttt{B1} \Rightarrow \texttt{A2}$ and
$\texttt{B1} \Rightarrow \texttt{A4}$ to construct \texttt{Segment
  \#1}.
%
In these two pairs, there are three instructions \texttt{B1},
\texttt{A2}, and \texttt{A4} which are annotated with
timestamps. Thus, we can arrange the instructions in \texttt{Segment
  \#1} accordingly.
%
...
In this way, a fuzzers mutates for all $g_i \in G$, and aggregate all
...
%
While we could naively test unexplored interleaving graphs in
$G_{derived}$ one-by-one, we choose to test multiple unexplored
interleaving graphs altogether.
