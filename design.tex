...
%
With the seed interleaving, we explore thread interleavings of a given
multi-thread input in the following three steps:\\[5pt]
\textbf{1) \textit{Decomposing}} an explored thread interleaving
into segments containing a small number of instructions.\\
\textbf{2) \textit{Mutating}} interleavings within segments to
generate unexplored interleavings for each segment.\\
\textbf{3) \textit{Recomposing}} mutated segments into whole thread
...
\begin{figure}[t]
  ...
  \caption{Example of recomposing mutated segments into whole
    ...
    interleaving to test \texttt{Segment \#1*} and \texttt{Segment
      \#2*}\yj{revision idea: add vertical bars for each column to make them appear steps.}}
  \label{fig:hint}
\end{figure}
