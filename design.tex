...
mutation. In other words, we first track coverage in the concurrency
dimension (shortly, concurrency coverage) from the given interleaving,
and generate other interleavings by rescheduling instructions for
...
%
One could argue that testing multiple pairs of conflicting access
operations is intractable since the size of the search space is too
large.
%
...
\PP{Overall steps}
%
We first propose a coverage metric, called \coverage, to
track multiple pairs of conflicting accesses.
%
\dr{explain \coverage}
%
\autoref{fig:overview} shows an example of how to track \coverage from
\autoref{fig:cve-2019-6974}.
%
As described \autoref{fig:overview}-(a), an interleaivng is given with
the execution order of
$\texttt{A1} \rightarrow \texttt{A2} \rightarrow \texttt{B1}
\rightarrow \texttt{B4} \rightarrow \texttt{B5} \rightarrow
\texttt{A3}$, and there are three pairs of conflicting accesses such
that $\texttt{A1} \rightarrow \texttt{B1}$,
...
After tracking \coverage from the given interleaving, we determine
whether the interleaving is worth further mutation.
%
Most coverage-guided fuzzing techniques determine that an input is a
subject of mutation if the input exhibits new coverage. In the context
of concurrency fuzzing, an interleaving is a subject of mutation if
new concurrency coverage (\eg, \coverage) is observed.
%
We however adopt a different strategy. We \textit{hypothesize which
  \coverage may be newly discovered} if we mutate an interleaving.
%
Since the execution order of conflicting accesses may affect an
outcome of a program, for each coverage, changing the execution order
of conflicting accesses yields \coverage that is possibly observed
with other interleavings, called \coverage candidate.

we \textit{estimates the universal coverage set $U$}, a set of
\coverage that the given concurrent jobs possibly execute.
%
Therefore, if the universal coverage set $U$ contains coverage that
are not yet observed, we determine that the interleaving is worthy of
further mutating.




\autoref{fig:overview}-(c) describes a subset of inferred \coverage
candidate $U$.
%
Taking the example of \texttt{Segment 1}, we infer \texttt{Segment 1'}
as a \coverage candidate by flipping the execution order of
$\texttt{A2} \rightarrow \texttt{B4}$ to
$\texttt{B4} \rightarrow \texttt{A2}$.
%
\dr{}
%
Similary, we generate \texttt{Segment 2'}
...
If an interleaving is worth further testing, we \textit{mutate the
  interleaving} by rescheduling memory access operations in the given
interleaving.

And then, to produce other interleavings, our interleaving mutation is
\textit{directed towards undisclosed coverages in $U$}.
