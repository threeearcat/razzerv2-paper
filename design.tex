...
%
Then, for two vertices $v_{x1}$ and $v_{x2}$ such that $i_{x1}$ was
executed before $i_{x2}$, a fuzzer generates directed edges $v_{x1}$
$\rightarrow$ $v_{x2}$ if 1) $v_{x1}$ and $v_{x2}$ were executed in
the same thread (\ie, a program-order edge), or 2) $v_{x1}$ and
$v_{x2}$ accessed the same memory object (\ie, an interleaving-order
edge).
%
...
\dr{I keep refining belows}
%
Afterwards, a fuzzer computes $g_i$, a subgraph of $g$, that contains
two interleaving-order edges.
%
For all
$e_z (v_{y1} \rightarrow v_{y2}), e_w (v_{y3} \rightarrow v_{y4}) \in
E$ where both $e_z$ and $e_w$ are interleaving-order edges and
$e_z \neq e_w$, a fuzzer constructs an interleaving graph
$g_i = (V_i, E_i)$, where $V_i$ is a set of
$\{v_{y1}, v_{y2}, v_{y3}, v_{y4}\}$, and $E_i$ ia set of edges
...
%
We can then re-compose a new thread interleaving by
...
over mutated segment graphs contained in given $S_{mutated}$, and
