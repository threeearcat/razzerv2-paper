...
access shared memory objects. And thus, these vertices are not
interesting when tracking thread interleaving, and a fuzzer discards
such vertices.
%
...
%
Lastly, a fuzzer gathers segment graphs into a set of segment graphs
...
\subsection{Directed Interleaving Search}
\label{ss:scheduler}
%
...
systematically and effectively explore thread interleavings.
...
%
Suppose $G = \{g_1, g_2, ..., g_n \}$, a set of \textit{explored}
segmeng graphs is given for a thread interleaving.
%
For each $g_i \in G$, we can then enumerate possible interleavings
inside $g_i$ called mutations of $g_i$.
%
By hashing mutation of $g_i$ and comparing it to collected \intcov, a
fuzzer can identify whay mutations of $g_i$ have not been explored
yet.
...
graph to derive other segment graphs.
%
We \textit{flip} interleaving-order edges (\ie, solid arrows) in a
segment graph described in \autoref{fig:interleavingmutation}-(a),
resulting in different segment graphs.
%
...
$\texttt{B1} \Rightarrow \texttt{A4}$ to generate a segment
...
\autoref{fig:interleavingmutation}-(d); a loop in a segment graph
indicates a contradiction on the execution order that any vertex on
the loop should be executed before itself.



In this way, a fuzzer generates mutations of all $g_i \in G$, which
include both explored and unexplored segment.
%
To exclude explored mutations, for each mutation $g^*$, a fuzzer
calculates its hash value, and compare it to collected \intcov. If the
hash value has not been recorded in the hash table, a fuzzer includes
$g^*$ into a set of mutations $G_{mutated}$.
%
In the next step, a fuzzer utilizes $G_{mutated}$ to determine what
thread interleaving to run in future iterations.
...
segment graphs in \autoref{fig:hint}-(a), and \autoref{fig:hint}-(b)
represents examples of their mutations, \texttt{Segment \#1*} and
\texttt{Segment \#2*}.
%
