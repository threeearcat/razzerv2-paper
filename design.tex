...
In order to explore thread interleavings of a given multi-thread input
(\eg, \texttt{sendmsg()} and \texttt{setsockopt()} in
\autoref{fig:cve-2017-17712}), our approach takes an \textit{explored}
thread interleaving as a seed interleaving.
%
With the seed interleaving, we explore thread interleavings of the
multi-thread input in three steps:
%
...
\item \textbf{\textit{Mutating}} decomposed segments to infer
  unexplored interleaving of a small number of instructions
...
%
Then, we illustrate how we utilize decomposed interleaving segments in
designing an interleaving coverage metric which represents
...
that mutates segments and recomposes a new thread interleaving to try
in future iterations (\textbf{Design goal
...
three instructions, and identifies different (unseen) execution orders
...
%
In summary, segmentizing thread interleaving provides two remarkable
benefits to a fuzzer.
%
First, tracking explored interleaving segments is a befitting choice
to determine whether to further explore thread interleavings of a
multi-thread input.
%
...
accesses, a fuzzer can speculatively enumerate interleavings of these
memory accesses.
%
If a fuzzer figures out which of the enumerated interleavings have not
been explored, \textit{a fuzzer can straightly explore the speculative
...
In order to decide which thread interleaving to run, we
