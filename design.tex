...
\autoref{fig:keyidea}-(a).
%
The first step is to represent instruction interleavings as a DAG. 
Since the execution in \autoref{fig:keyidea}-(a) has five memory
accesses, a fuzzer generates five vertices where each corresponds 
to a memory-accessing instruction as described in
...
Afterwards, a fuzzer derives subgraphs from the generated DAG, where
each graph has two interleaving-order edges. Therefore, 
a subgraph represents  segment 
%
