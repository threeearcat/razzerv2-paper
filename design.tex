...
%
Semantically, segment graphs represent interleaving orders (via
interleaving-order edges) of at most four instructions (via vertices),
and different interleavings (\eg, \autoref{fig:keyidea}-(a).
%
If a fuzzer collects all segment graphs from a multi-thread input, a
fuzzer can confidently conclude that the input no more reveals an
interesting combination of interleaving orders.
%
Therefore, our fuzzer choose to collect explored segment graphs as
interleaving coverage metric.
