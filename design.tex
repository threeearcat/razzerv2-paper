...
of doing a randomized search (as in most previous approaches), if a
...
\begin{figure}[t]
  ...
  \caption{Example of recomposing mutated segments into whole
    interleaving, where (a) represents mutated segments, (b)
    represents a graph combining the two mutated segments, and (c)
    represents a sequence of instructions to test \texttt{Segment
      \#1*} and \texttt{Segment \#2*}}
  \label{fig:hint}
  \vspace{-5pt}
\end{figure}
