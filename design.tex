...
Throughout the rest of this paper, we denote an interleaving fragment
as a subpart of an interleaving that contains four memory access
operations, \dr{}
%
...
However, it is worth noting that many fragments are not exclusive.
%
In ..., XXX and XXX can be executed in one interleaving.

If we select an interleaving to test in a \textit{smart} way, we can
quickly consume a universal fragment set $U$.  Therefore, we can
drastically reduce the number execution to find all biconflict coverage.
