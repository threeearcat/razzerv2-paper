...
% thread (thread~B) if thread~B reads a value written by thread~A, the
% memory accesses causing a race condition should conflict with each
% others.
%
\dr{Why and how we translate four memory accesses to two conflicts}
%
\dr{keywords: requirement of interactions}
%
Therefore, 
