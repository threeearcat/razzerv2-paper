...
mutated segment contains interleavings that is to be tested. 
...
scheduling points where each interleaving happens from the
...
segment graphs $G_{mutated}$, and utilizes $G_{mutated}$ to determine
how to schedule instructions in future iterations.





\PP{Recomposing mutated segment graphs into large graph}
%
One could select one mutated segment graph in $G_{mutated}$ and run an
input while enforcing interleaving orders described in the segment
graph.
%
...
and \textit{recompose} them into a large graph to test multiple
mutated segment graphs at one.
