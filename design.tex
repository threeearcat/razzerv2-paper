...
(\eg, a seed interleaving) of the concurrent job are given as an
input. We also assume that all memory access operations are totally
ordered.
...
Throughout the rest of this paper, we name an interleaving fragment as
a subpart of an interleaving that represents two scheduling
contraints. For example, the race condition in
\autoref{fig:cve-2019-6974} is caused by an interleaving fragment
$(\texttt{A1} \rightarrow \texttt{B1}) \wedge (\texttt{B4} \rightarrow
\texttt{A2})$.


\dr{univeral fragment set U}
...
%
If two threads conflict on $N$ pairs of accesses, the number of two
pairs of conflicting accesses is propotional to $N^2$, which is
possibly too large to test one-by-one.

However, it is worth noting that many interleaving fragments are not
exclusive.
%
When we execute a concurrent job once yielding an interleaving of
memory access operations, we can observe many interleaving fragments
simultaneously in the interleaving.
%
Based on the observation, we can generate instructions in a
\textit{smart} way.
%
In other words, if we know which fragments are not yet disclosed from
a universal fragment set $U$, we can schedule instructions to
experience undisclosed fragments.
...
its scheduling constraints are
$(\texttt{A1} \rightarrow \texttt{B1}) \wedge (\texttt{B4} \rightarrow
\texttt{A2})$, but \texttt{A2} is executed before \texttt{B4}.

With the given input, the first task is to track biconflict coverage.
In the given interleaving of \autoref{fig:overview}-(a), there are
three individual pairs conflicting accesses,
