...
Razzer~\cite{razzer} uses static analysis to 
identify memory-accessing instruction pairs of the same objects, and 
Snowboard~\cite{snowboard} uses dynamic analysis. 
From the detected instruction pairs, they perform heuristic approaches
to obtain new thread interleaving. Alas, they are coverage-oblivious, 
so they cannot prune unnecessary executions where the outcome 
of a thread interleaving result in the same kind. 
We emphasize that 
