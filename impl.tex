...
and timestamp-annotated memory accesses executed by each system call
with a support from instrumentation detailed in
\autoref{ss:instrumentation}.
...
$I_{ST}$, the single-thread executor draws a interleaving graph $g$
from total memory accesses executed by $S_i$ and $S_j$.
%
Then, the single-thread executor decomposes $g$ into a set of segment
graphs $G$ (as described in \autoref{ss:coverage}), and mutates
$g_i \in G$ to derive a set of mutated segment graphs $G_{mutated}$
