...
call for probing interleaving segments.
%
...
\begin{figure}
  ...
  \caption{An overview of \sys's architecture. \dr{interleaving coverage is missing}}
  \label{fig:workflow}
\end{figure}
...
Transforming $I_{ST}$ into $I_{MT}$ is simple; XXX
%
...
After executing $I_{MT}$, the multi-thread executor collect memory
accesses that racing system calls executed.
%
With these memory accesses, the multi-thread executor probes for
interleaving segment coverage. In addition, it derives more unobserved
interleaving segments, and feeds them to the multi-thread generator to
further explore thread interleavings.
