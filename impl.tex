...
%
And, we implement the callback functions into the Linux kernel source
tree.
%
...
functions).
%
For the rest, \sys modifies existing frameworks as follows: QEMU to
implement execution engine~(\autoref{ss:engine}) with 1662 LoC in C,
and Syzkaller to implement the userspace fuzzer~(\autoref{ss:fuzzer})
with 3334 LoC in GoLang and 341 LoC in C++.
