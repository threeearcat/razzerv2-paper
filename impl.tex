...
userspace fuzzer sends a schedule to run before executing system
calls.
%
...
scheduling point (\ie, \C{2}-1).
%
After all threads call \texttt{hcall_ready()}, the execution engine
wakes up them, and the threads start executing system calls (\ie,
\C{3}).
