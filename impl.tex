...
Zinterleaving coverage, and hands it to the next stage.
...
%
When generating a single-thread input, \sys randomly generates a
system call sequence based on well-formed system description grammar
...
single-thread input, and modifies the single-thread ipnut. It may
append additional system calls, remove existing system calls, or
change values of arguments, while following \texttt{Syzlang}.


\PP{Single-Thread Executor}
%
After the single-thread generator provides a single-thread input
$I_{ST}$, the single-thread executor runs the input while tracing
...
Specifically, traced basic blocks are used to track code coverage. For
this, \sys uses branch coverage as code coverage.
%
After executing $I_{ST}$, 
