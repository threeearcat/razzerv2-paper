...
fuzzing similar with recent concurrency fuzzers, Razzer~\cite{razzer}
...
\textit{sequentially}. However, the single-thread executor still can
...
unobserved before in $G^{*}_{(i,j)}$.
%
Lastly, if $G^{*}_{(i,j)}$ is not empty, the single thread
executor passes $I_{ST}$ as well as ($S_i$, $S_j$) and
$G^{*}_{(i,j)}$ to the next phase, the multi-thread fuzzing.
...
$G^{*}_{(i,j)}$, the \textit{multi-thread generator} transforms
...
$I_{ST}$, the input transformation is simply done by splitting
$I_{ST}$ into two parts, XXX and YYY.
%
...
$G^{*}_{(i,j)}$ as input.
%
The multi-thread generator selects interleaving segment graphs from
$G^{*}_{(i,j)}$
...
%
In addition, memory accesses should be annotated with timestamps.
%
...
%
\texttt{trace_access()} then records these five arguments into
...
wakes them up, and the threads start executing system calls (\C{3}).
