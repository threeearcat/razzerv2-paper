...
call to track interleaving segments.
%
...
trace memory accesses, and the execution engine~(\autoref{ss:engine})
to control thread schedulings.
%
...
In \sys, both single-thread fuzzing multi-thread fuzzing consist of two components, an input generator and an
...
%
More specifically, for each system call pair ($S_i$, $S_j$), the single-thread
executor generates interleaving segment graphs from memory accesses of the system call pair.
%
While $S_i$ and $S_j$ was not executed concurrently, the single-thread executor does not record generated interleaving segment graphs as interleaving segment coverage, but make us of them to derive unobserved interleaving segment graphs.
%
If XXX, then the single-thread executor hands $I_{ST}$, ($S_i$, $S_j$), and XXX to the multi-thread fuzzing.


\subsubsection{Multi-thread fuzzing}
\label{sss:multithreadfuzzing}
%
After $I_{ST}$ is handed with ($S_i$, $S_j$) and XXX, the multi-thread generator transforms $I_{ST}$ to a
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.8\linewidth]{fig/schedule.pdf}
...
%
The purpose of the multi-thread executor is to run $I_{MT}$ while enforcing a schedule given generated by the multi-thread generator.
%
To this end, the multi-thread executor invokes hypercalls to deliver a schedule to the execution engine before executing system calls of $I_{MT}$.
