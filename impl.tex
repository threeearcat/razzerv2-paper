...
\sys is a coverage-directed concurrency fuzzer to effectively find
concurrency bugs.
%
...
components, the userspace fuzzer~(\autoref{ss:fuzzer}), target kernel
instrumentation~(\autoref{ss:instrumentation}), and the execution
engine~(\autoref{ss:engine}), and implemetation of
\sys~(\autoref{ss:impl}).
