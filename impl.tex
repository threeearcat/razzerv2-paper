...
\sys is a concurrency fuzzer tailored to effectively discover kernel
concurrency bugs, where its key improvement lies in adopting
interleaving segment coverage and the speculative interleaving
exploration described in \autoref{s:design}.
%
...
\begin{figure}
  ...
  \caption{An overview of \sys's architecture. \dr{unobserved -->
      unexplored}}
  \label{fig:workflow}
\end{figure}
...
the single-thread executor considers the sequential execution of $S_i$
and $S_j$ as a seed interleaving, and draws an interleaving graph from
the seed interleaving.
%
To derive segment graphs that have not been explored, then the
single-thread executor decomposes the interleaving graph into a set of
segment graphs $G$ (as described in \autoref{ss:coverage}), and
mutates segment graphs $g_i$ contained in $G$ to construct a set of
unexplored mutated segment graphs $G_{mutated}$ (as described in
...
identify basic blocks and memory accesses executed by each system
call.
...
scheduling point indicates (\ie, \C{2}-1).
%
...
engine performs preemption (\ie, \C{4}) by suspending the running
