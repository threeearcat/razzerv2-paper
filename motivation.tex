...
untested~\cite{krace, conzzer, muzz}\yj{use clearer sentence},
%
or \textbf{2)} waste the computing power by ineffectively exploring
the search space of thread interleaving~\cite{snowboard, razzer}.
...
performing randomized search pervasively used in previous approaches~\cite{ski, krace, pctalgorithm}\yj{cite}.
...
that their interleaving coverage metrics and thread scheduling control
...
less-informative 
%to determine whether a multi-thread input needs to be further tested. 
...
interleavings taken place in the same function pair\yj{I do not get this}.
...
%saturate existing interleaving coverage
%
