...
thread interleavings, missing the uninitialized access bug.
%
...
explored before, then, a fuzzer can utilize interleaving coverage to
determine which interleaving to search in future iterations (\ie,
$\texttt{A2} \Rightarrow \texttt{B1} \Rightarrow \texttt{A4}$).
%
As a consequence, a fuzzer can quickly discover the uninitialized
access bug.
...
\PP{Less-informative coverage metric}
%
We find that previously proposed interleaving coverage metrics are not
informative enough to determine
...
% tracking individual interleaving
...
\PP{Coverage-irrelevant interleaving search strategy}
%
