...
multi-thread input in which a concurrency bug reside~\cite{krace,
  conzzer}, \yj{use clearer sentence}
% suffers from
...
instructions, \texttt{A2}, \texttt{A4}, and \texttt{B1}, access the
same memory object~(\ie, \texttt{inet->hdrincl}), and are executed in
the order of
...
value written by $I_W$ and they are executed by different threads.
...
sees two unexplored directed-instruction pairs (\ie,
...
behaviors of \texttt{Interleaving \#2}.
%
As mentioned above, alias coverage captures a single
directed-instruction pair.  But in \texttt{Interleaving \#2}, there is
no unobserved directed-instruction pair (\ie,
...
pair, it is not aware of interleavings inside a function.
%
And, the two thread interleavings in \autoref{fig:alias-coverage} take
place in the same function (\ie, \texttt{A2} and \texttt{A4} are
executed in the same function).
%
Thus, concurrent call pair also shows the same limitation as alias
coverage when it comes to discovering the uninitialized access bug.
...
%
As a consequence, none of existing approaches effectively search
thread interleavings to run in future iterations.
