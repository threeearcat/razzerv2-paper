...
However, according to the alias coverage, \texttt{Interleaving \#2}
does not contain any new coverage because $\texttt{B1} \Rightarrow \texttt{A4}$) is already observed in \texttt{Interleaving \#1}).
KRace considers that growing the alias coverage is a promising signal 
to explore thread interleavings further, but the \texttt{Interleaving \#2} does not increase the alias coverage in this scenario.
...
\PP{Inefficient interleaving search strategy}
%
...
Razzer~\cite{razzer} and Snowboard~\cite{snowboard} are coverage-oblivious. The do not make use of any interleaving coverage 
and perform their randomized search.
%
Krace~\cite{krace} also explores thread interleaving orders randomly without
considering what thread interleavings are explored before. 
Krace limitedly utilizes interleaving coverage only when deciding whether it runs a given multi-thread input more. Whereas, we use 
the coverage to infer which thread interleavings to search further
