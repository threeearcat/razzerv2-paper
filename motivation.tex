...
\PP{Combined interleaving orders}
%
...
denotes the execution order of a instruction pair that access the same
memory object.
%
In the example of \autoref{fig:cve-2019-6974}, two interleaving orders
such as 1) $\texttt{A2} \Rightarrow \texttt{B1}$~\footnote{In this
  paper, $\texttt{X} \Rightarrow \texttt{Y}$ denotes that \texttt{X}
  is executed before \texttt{Y}} (to make thread~A not initialize
\texttt{rfv}), and
%
2) $\texttt{B1} \Rightarrow \texttt{A4}$ (to make thread~A dereference
uninitialized \texttt{rfv}) cause the uninitialized access bug.
%
In order words, among all thread interleavings, ones that satisfy a
conjunctive condition (\ie,
$(\texttt{A2} \Rightarrow \texttt{B1}) \wedge (\texttt{B1} \Rightarrow
\texttt{A4})$) cause the concurrency bug, while all other
