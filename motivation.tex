...
\dr{Not-defined terms: multi-thread input}
...
\end{figure}
%
\autoref{fig:cve-2019-6974} describes how an erroneous thread
...
denotes the execution order of an instruction pair that access the
same memory object.
%
In the example of \autoref{fig:cve-2019-6974}, two interleaving orders
...
\texttt{A4})$) causes the uninitialized access bug to manifest.




This observation provides an intuition that the progress of the thread
interleaving exploration should be described by considering
combinations of interleaving orders.
%
In other words, if the combination of interleaving orders
$(\texttt{A2} \Rightarrow \texttt{B1}) \wedge (\texttt{B1} \Rightarrow
\texttt{A4})$ has not been observed in \autoref{fig:cve-2019-6974}, a
fuzzer should consider that there are more interesting thread
interleavings and strive to further explore the search space of thread
interleaving.
...
%
\item \emph{A fuzzer should be able to determine that interesting
...
In \autoref{fig:cve-2019-6974}, \textbf{R1} states that a fuzzer
should adopt a proper interleaving coverage metric that can be used to
determine whether
$(\texttt{A2} \Rightarrow \texttt{B1}) \wedge (\texttt{B1} \Rightarrow
\texttt{A4})$ remains untested or not.
%
\textbf{R2} describes that if this conjunctive condition has not been
tested, a fuzzer needs to schedule instructions to quickly cover the
conjunctive condition instead of exploring the entire search space of
thread interleaving randomly.
...
\begin{figure}[t]
  ...
  \caption{Two thread interleavings between thread~A and thread~B
    described in \autoref{fig:cve-2019-6974}. Regarding
