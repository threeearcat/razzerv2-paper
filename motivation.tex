...
executes the two system calls \textit{sequentially} such that thread~A
executes all instructions followed by the execution of thread~B.  In
the explored execution, a fuzzer observes the three instructions,
\texttt{A2}, \texttt{A4}, and \texttt{B1}, are executed in the order
of $\texttt{A2} \Rightarrow \texttt{A4} \Rightarrow \texttt{B1}$ and
...
%
\dr{history-based? or coverage-based?}
%
...
approaches~\cite{ski, krace, pctalgorithm, muzz}.%\yj{cite}
%
\blue{
  %
