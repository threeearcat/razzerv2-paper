...
\dr{Not-defined terms: interleaving order, multi-thread input}
...
%
To sum up, among all possible thread interleavings, only ones that
\textit{conjunctively satisfy the two interleaving orders, \ie,
  $(\texttt{A2} \Rightarrow \texttt{B1}) \wedge (\texttt{B1}
  \Rightarrow \texttt{A4})$}, can cause the concurrency bug while and
all other thread interleavings cannot.
