...
%
An interleaving coverage metric should be able to determine whether
the combined interleaving orders
%
In other words, 

Semantically, if interleaving coverage is saturated, the combined
interleaving orders
$(\texttt{A2} \Rightarrow \texttt{B1}) \wedge (\texttt{B1} \Rightarrow
\texttt{A4}))$ should be executed.
%
Otherwise, a fuzzer may move on to other inputs, and miss the
...
\PP{Design goal 2: History-based thread interleaving search strategy}
%
Based on the observation 2, we elicit a design goal of a thread
interleaving search strategy.
%
Particulary, a thread interleaving search strategy should leverage a
