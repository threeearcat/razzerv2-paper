...
\PP{Design goal 1: combinatorial interleaving coverage metric}
%
...
%
As a consequence, tracking single interleaving orders is not enough
for an interleaving coverage metric.
...
\begin{figure}[t]
  ...
  \caption{Two thread interleavings between thread~A and thread~B
    ...
    \texttt{inet->hdrincl}, all four interleaving orders are observed
    ($\texttt{A2} \Rightarrow \texttt{B1}$ and
    $\texttt{A4} \Rightarrow \texttt{B1}$ in
    \texttt{Interleaving~\#1}, $\texttt{B1} \Rightarrow \texttt{A2}$,
    and $\texttt{B1} \Rightarrow \texttt{A4}$ in
    \texttt{Interleaving~\#2}), but the uninitialized access bug does
    not manifest.}
...
coverage metric tracks single interleaving orders.
%
In the two interleavings described in this figure, we can observe all
four interleaving orders with respect to \texttt{inet->hdrincl}, while
the uninitialized access bug does not manifest.
%
Therefore, if an interleaving coverage metric tracks single
interleaving orders, the coverage metric can be saturated even though
a concurrency bug does not manifest, and a fuzzer may miss a
concurrency bug.
%
...
%
Interestingly, an execution that does not cause the uninitialized
access bug gives a hint as to which thread interleavings should be
further explored.
%
...
access bug, we can observe that three memory access operations,
...
$\texttt{A2} \Rightarrow \texttt{B1} \Rightarrow \texttt{A4}$).
%
And this imaginary interleaving is what we are looking for; it is
exactly the condition that causes the uninitialized access bug, and if
we execute the imaginary interleaving, we can discover the
uninitialized access bug.



\PP{Design goal 2: effective thread scheduling control mechanism}
%
...
\begin{table}[t]
  ...
  \caption{Recent fuzzing works to discover concurrency bugs in the
    ...
    metric.
    %
    \dr{MUZZ and Conzzer do not focus on the kernel. Decide put them
      here or not.}}
