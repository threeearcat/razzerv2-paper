...
\PP{Design goal 1: descriptive interleaving coverage metric}
%
This observation provides an insight that an interleaving coverage
metric should consider a combination of 
...
further explored.
%
Let us assume we execute the two system calls \textit{sequentially}
such that the execution of thread~A is followed by the execution of
thread~B.
%
...
$(\texttt{A2} \Rightarrow \texttt{A4}) \wedge (\texttt{A4} \Rightarrow
\texttt{B1})$.
%
Unsuprisingly, we can easily imagine a thread interleaving by flipping
the execution order between \texttt{A4} and \texttt{B1}.
%
And this imaginary thread interleaving is what we are looking for; it
satisfies the combination of interleaving orders,
$(\texttt{A2} \Rightarrow \texttt{A4}) \wedge (\texttt{B1} \Rightarrow
\texttt{A4})$ that causes the uninitialized access bug.



\PP{Design goal 2: effective thread scheduling control mechanism}
%
Based on the observation 2, we elicit a design goal 
