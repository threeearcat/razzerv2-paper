...
that each input explores, and various sanitizers detect
vulnerabilities~\cite{meds, kasan, asan, ubsan, lockdep}.
...
take consider of the \textit{concurrency aspect} of the kernel
execution into the fuzzing process.
%
Concurrency fuzzers generally repeat the execution of a multi-thread
input (\eg, a multi-threaded system call sequence) while controlling
thread scheduling and tracking unique behaviors of thread
interleavings.
...
To explore diverse thread interleavings for each iteration,
concurrency fuzzers adopt various thread interleaving exploration
schemes.
%
For instance, for every iteration, Razzer~\cite{razzer} and
...
%
Concurrency fuzzers utilize interleaving coverage as a signal to
determine if a multi-thread input needs to be further tested.
