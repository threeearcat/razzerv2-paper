...
\texttt{B4}, and 2) \texttt{B1} $\rightarrow$ \texttt{A3}.
%
Assuming \texttt{n_hdlc->tbuf} is not \texttt{NULL} before the
execution, thread A always executes \texttt{A2} if \texttt{A1} is
executed before \texttt{B4}. Similarly, thread B always executes
\texttt{B3} if \texttt{B1} is executed before \texttt{A3}. Therefore,
if the two conditions are met, \texttt{n_hdlc->tbuf} is inserted into
a linked list \texttt{free_list} at both \texttt{A2} and \texttt{B3},
causing a list corruption.
%
