...
%
1) capturing unique behaviors of thread interleavings with a
\textit{interleaivng coverage metric}, and
%
2) determining which thread interleave to run on each iteration with a
\textit{thread interleaving search strategy}.
...
coverage metrics.
%
Interleaving coverage metrics are used to track the execution order of
concurrent events between threads from a finer-granularity (\eg,
...
Snowboard~\cite{snowboard} determines the execution order of a single
pair of instructions, and Krace~\cite{krace} randomly determins a
thread interleaving to run.
%
Whereas Conzzer~\cite{conzzer} designates two functions to run
concurrently.
%
\dr{MUZZ?}
...
After determining a thread interleaivng to run, concurrency fuzzing
overrides the kernel scheduler, and controls thread scheduling as
desired.
%
To this end, various thread scheduling control mechanisms are used,
such as suspending/resuming virtual CPUs~\cite{razzer, snowboard}, or
injecting delays via instrumentation~\cite{krace} or the breakpoint
feature~\cite{conzzer}.
%
While these mechanisms are designed differently, all of them allow a
