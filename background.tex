...
Since the kernel is a de facto security basis of an entire system, it
is paramount to eliminate vulnerabilities in the kernel to improve the
reliability and security of a system.
%
Therefore, a number of fuzzing approaches~\cite{imf, syzkaller,
  moonshine, hfl, healer, janus, hydra, trinity} have been proposed to
discover vulnerabilities in the kernel as the first step to secure the
system.


These fuzzing approaches, specifically coverage-guided fuzzing
approaches, generally execute a massive number of inputs while
tracking which code path each input explores.
%
...
%
In addition, during the execution of such inputs, a fuzzer discovers
...
interleaving coverage metric to capture interesting behaviors of
thread interleavings, and a thread scheduling control mechanism to
tame the non-deterministic behavior of thread interleaving.
...
%
As Krace~\cite{krace} reveals, code coverage metrics are not enough to
capture interesting behaviors of thread interleavings, because code
coverage metrics only track the sequential aspect of a program (\eg,
branch coverage tracks whether a branch is taken or not) without
paying attention to thread interleavings.



To capture interesting behaviors of thread interleavings, a few
concurrency fuzzing approaches come up with interleaving coverage
metrics.
%
Especially for the kernel, Krace proposes alias coverage which tracks
inter-thread data flow between a pair of instructions. In other words,
alias coverage tracks a directed pair of instructions
...
\PP{Thread scheduling control mechanism.}
%
Concurrency fuzzing considers thread scheduling is another input
domain that a fuzzer should explore.
%
To this end, concurrency fuzzing consists of a thread scheduling
control mechanism to override the uncontrollable kernel scheduler.
