...
example, a race condition occurs in the binder device driver, which is
an IPC framework for Android. Specifically, a NULL-dereference bug
manifests when two system calls are executed concurrently:
\texttt{mmap()} to map the device driver into a user address space and
...
%
In that case, \texttt{alloc->vma} is not \texttt{NULL} when thread~B
executes \texttt{B1} while \texttt{alloc->mm} is \texttt{NULL} at
\texttt{B3}.
%
...
%
It is worth noting that the only difference between the two inputs is
that in \texttt{Input 1}, \texttt{ioctl(CREATE_VCPU)} executes
\texttt{A1} at last while \texttt{ioctl(CREATE_DEVICE)} in
\texttt{Input 2} executes \texttt{A1} at first. In both inputs,
thread~B conducts the same task.
%
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.8\linewidth]{fig/alias-coverage-interleaving.pdf}
  \caption{}
  \label{fig:alias-coverage-interleaving}
\end{figure}

However, if we adopt alias coverage as the coverage metric in the
concurrency dimension, a fuzzer may not recognize that their
interleavings exhibit different behaviors.
%
If a fuzzer executes \texttt{Input 1} with various interleavings as
described \autoref{fig:alias-coverage-interleaving}, a fuzzer observes
all execution order of single pair of conflicting accesses. In other
words, a fuzzer observes $\texttt{A1} \rightarrow \texttt{B1}$,
$\texttt{A2} \rightarrow \texttt{B4}$ (in run 1),
$\texttt{B1} \rightarrow \texttt{A1}$ (in run 2), and
$\texttt{B4} \rightarrow \texttt{A2}$ (in run 3).
%
Therefore, when executing \texttt{Input 2} (in run 4), a fuzzer may
not collect new alias coverage, and as Krace states, a fuzzer may
de-prioritize \texttt{Input 2}. Consequently, a fuzzer may miss the
race condition.
%
