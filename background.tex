...
bugs~\cite{learningfrommistakes}, most race conditions manifest only
depending on the execution order of the four memory accesses. \yj{how
  to understand this sentence}.
%
Specifically, 92\% of race conditions the authors studied are
guaranteed to manifest if at most four memory accesses are executed in
...
example the use-after-free bug is triggered if the execution order of
four memory access operations satisfies the certain condition such
that \texttt{A1} is executed before \texttt{B2} and \texttt{B1} is
executed before \texttt{A2}.
%
...
%
If a coverage metric does not capture whether an input has a potential
to trigger a race condition, a fuzzer may ignore inputs in which there
are unexplored interleavings, or waste the computing power to
valueless inputs.
%
In this regard, our primary question is whether existing coverage
metrics in the concurrency dimension are suitable to track
interleavings potentially causing a race condition, for example,
$(\texttt{A1} \rightarrow \texttt{B2}) \wedge (\texttt{B1} \rightarrow
\texttt{A2})$ in \autoref{fig:cve-2017-10661}.
...
%
Taking alias coverage as an example, \autoref{fig:alias-coverage}
shows two interleavings of \autoref{fig:cve-2017-10661} that capture
all alias coverage, but the race condition does not manifest.
%
As the figure describes, if we execute the two threads
\textit{sequentially}, a fuzzer is able to track inter-thread data
flow (\ie, alias coverage) $\texttt{B2} \rightarrow \texttt{A1}$ and
$\texttt{A2} \rightarrow \texttt{B1}$.
%
After executing these two interleavings, as Krace states, a fuzzer may
de-prioritize this input as it tracks all possible alias coverage in
regard to \texttt{ctx->might_cancel}, and consequently, the race
