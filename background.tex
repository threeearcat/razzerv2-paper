...
Recently, several approaches~\cite{razzer, krace, snowboard, muzz,
  conzzer} have been proposed to take consider of the
\textit{concurrency aspect} of the execution into the fuzzing process.
%
Concurrency fuzzers generally repeat executing a multi-thread input
(\eg, a multi-threaded system call sequence) while 1) capturing unique
behaviors of thread interleavings with a \textit{interleaivng coverage
  metric}, and 2) taming the non-determinitsic behavior of thread
interleavings using \textit{thread scheduling control}.
...
\PP{Thread scheduling control.} \dr{split this into two?}
%
...
concurrency fuzzing overrides the kernel scheduler, and controls
thread scheduling as desired.



To this end, concurrency fuzzing consists of a thread scheduling
control mechanism, and a thread interleaving search strategy.
%
A thread scheduling control mechanism is used to control thread
scheduling as a fuzzer wants.
%
To this end, various methods are used; Razzer~\cite{razzer} and
Snowboard~\cite{snowboard} suspends and resumes virtual CPUs on which
target threads are running, Krace~\cite{krace} injects delays before
...
A thread interleaving search strategy determines which thread
interleaving to run for each execution.
%
For instance, Krace~\cite{krace} randomly determins a thread
interleaving to run, Snowboard~\cite{snowboard} and
Razzer~\cite{razzer} enforces an interleaving order of a single pair
of instructions, and \dr{Conzzer does this, and MUZZ does that}.
