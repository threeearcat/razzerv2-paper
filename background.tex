...
\dr{keep in mind: concurrency fuzzing to expose erroneous behaviors}
...
daunting task.
%
It is mainly due to two challenges: 1) the number of possible
interleavings grows exponentially according to the number of memory
access operations, making exhaustively exploring the interleaving
space is an intractable problem, 2) concurrency bugs inherently stem
from the non-deterministic behavior.

In the perspective of fuzzing, the two challenges are addressed by a
concurrency coverage metric and a customized scheduler respectively.
%
...
However, we found that prior works fall short in finding concurrency
bugs mainly due to that they do not consider when concurrency bugs
manifest.
%
In other words, a coverage metric of prior works do not answer the
question of whether there are remaining interleavings that potentially
causes concurrency bugs, and a customized scheduler .
%
...
% Want to say: there are race conditions that cannot be detected using
% a data race detector. Need to effectively explore the interleaving
% space to observe the outcome caused by various interleavings.
