...
\begin{figure}
  \centering
  \includegraphics[width=0.9\linewidth]{fig/cve-2017-2636.pdf}
  \caption{CVE-2017-2636}
  \label{fig:racecondition}
\end{figure}  

\autoref{fig:racecondition} explains the above survey. In this example
the use-after-free bug is triggered if two execution orders are
conjunctively fulfilled such that 1) \texttt{A1} $\rightarrow$
...
%
If we execute the two threads \texttt{sequentially}, we can observe
inter-thread data flow from \texttt{B2} to \texttt{A1}, and
\texttt{A2} to \texttt{B1}.
%
According to Krace, a fuzzer tracks all possible alias coverage in
regard to \texttt{ctx->might_cancel}, and de-prioritizes the two
threads as it thinks there will be no more interesting behavior.
%
Consequently, the race condition may be missed.

%
\dr{At this point, I'm worrying that one could argue that this bug can
  be found with a data race detector even if the race condition does
  not manifest. That is why I keep thinking about saying the
  difference between data race and race condition and then use
  cve-2019-6974 as an example.}
