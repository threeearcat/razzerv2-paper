...
concurrency fuzzing approaches~\cite{conzzer, krace} come up with
...
These fuzzing approaches aggregate interleaving coverage across
iterations while repeatedly executing a given multi-thread input (\ie,
a multi-threaded system call sequence), until the multi-thread input
no longer provides new interleaving coverage.
%
Taking Krace as an example, alias coverage constructs a set of
instruction pairs collected across iterations. When further iterations
do not generate a new instruction pair, Krace de-prioritizes the
...
The basic purpose of a thread scheduling control mechanism is to stall
the execution of a thread during the execution.
%
...
target thread is running, Krace~\cite{krace} injects a delay before
memory accesses through instrumentation, and Conzzer~\cite{conzzer}
injects a delay through the breakpoint feature.


While these methods are different, all of them allow a fuzzer to
control thread scheduling.
%
For instance, a fuzzer can specify exact instruction locations to
stall the execution~\cite{snowboard, razzer, conzzer}, or the amount
of delays injected at runtime~\cite{krace}.
