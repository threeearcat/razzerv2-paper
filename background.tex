...
accesses, but race conditions mostly are \textit{combined results of multiple pairs} of conflicting accesses.
%
...
depending on the execution order of a few memory access operations.
\yj{how to understand this sentence}.
%
Specifically, 92\% of race conditions the authors studied manifest if
at most four memory accesses operations are executed in a specific
order.
%
...
\begin{figure}[t]
  ...
  \caption{Simplified code snippet of CVE-2019-6974. A \texttt{BUG()}
    is executed if \texttt{A1} is executed before \texttt{B1} and
    \texttt{B4} is executed before
    \texttt{A2}. \texttt{atomic_fetch_add()} atomically increases the
    value by 1 and returns an old
    value. \texttt{atomic_dec_and_test()} atomically decreases the
    value by 1 and tests the result is 0.}
  \label{fig:cve-2019-6974}
\end{figure}

\autoref{fig:cve-2019-6974} explains the above survey.  In this
example, a race condition manifests when two system calls are executed
concurrently: \texttt{ioctl()} to create a virtual device in a virtual
machine and \texttt{close()} to shutdown the virtual device.
%
As a result of concurrent execution, a reference counter may be
incorrectly managed to increment from \texttt{0}. In Linux, this
indiates a use-after-free bug because if a reference count hits
\texttt{0}, the corresponding object is going to be freed.

The race condition happens as follows.  Suppose the initial value of
\texttt{kvm->refcnt} is \texttt{1}. During the creation of the virtual
device, thread~A erroneously makes the \texttt{file} object globally
visible (at \texttt{A1}) before it manipulates the reference counter
of \texttt{kvm} (at \texttt{A2}).
%
Therefore, thread~B reads non-NULL \texttt{file} at \texttt{B1} while
\texttt{kvm->refcnt} still remains \texttt{1}, and decreases
\texttt{kvm->refcnt} to \texttt{0} at \texttt{B4}.
%
After that, when thread~A increases \texttt{kvm->refcnt}, it executes
\texttt{BUG()} at \texttt{A3} because \texttt{kvm->refcnt} already
hits \texttt{0}.
%
...
% , and all other memory accesses do not affect the
...
\begin{figure}[t]
  ...
  \caption{Two inputs that are not distinguishable using alias
    coverage. For each input, a fuzzer executes two syscalls
    concurrently.}
...
%
Let us suppose we have two similar inputs consisting of two concurrent
syscalls.
%
As described in \autoref{fig:alias-coverage}, \texttt{Input \#1}
consists of \texttt{ioctl(CREATE_VCPU)} creating a virtual CPU and
\texttt{close()} closing the virtual CPU, and \texttt{Input \#2}
consists of \texttt{ioctl(CREATE_DEVICE)} creating a virtual device
and \texttt{close()} closing the virtual device.
%
Although they look similar, \texttt{Input \#1} is benign because the
logic to create the virtual CPU is correct (\ie, \texttt{kvm->refcnt}
is manipulated before making \texttt{file} globally visible) while
\texttt{Input \#2} causes the race condition (\ie, CVE-2019-6974) as
explained before.


However, if we adopt alias coverage as the coverage metric in the
concurrency dimension, the two inputs are not distinguishable.
%
After a fuzzer runs \texttt{Input \#1} with various interleavings,
a fuzzer observes all inter-thread data flow

\texttt{Input \#2} does not reveal new alias coverage



After executing these two interleavings, as Krace states, a fuzzer may
de-prioritize the offending input as it tracks all possible alias
coverage, and consequently, the race condition may be missed.
%
...
mechanism that quickly saturate the coverage metric, and
%
3) Lastly, we combine the coverage metric and the interleaving
mutation mechanism into Syzkaller~\cite{syzkaller} to find out race
conditions in the Linux kernel.
