...
unique behaviors of thread interleavings with a \textit{interleaivng
  coverage metric}, and 2) taming the non-determinitsic behavior of
thread interleavings through a \textit{thread scheduling control}.
...
Snowboard~\cite{snowboard} suspend and resume virtual CPUs on which
...
A thread interleaving search strategy is adopted for a fuzzer to
determine which thread interleaving to run for each iteration.
%
\dr{explain correctly: }For instance, Krace~\cite{krace} randomly
determins a thread interleaving to run, Snowboard~\cite{snowboard} and
