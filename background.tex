...
%
While executing a multi-thread input (\ie, a multi-threaded system
call sequence), alias coverage collects an instruction pair ($I_S$,
$I_L$), if a read instruction $I_L$ reads a value written by a write
instruction $I_S$, and they are executed by different threads.
%
...
These fuzzing approaches repeatedly execute a multi-thread input, and
aggregate interleaving coverage across iterations until the
...
A thread scheduling control mechanism stalls the execution of a thread
